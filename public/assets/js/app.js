...
const ROLE_TABS = {
  User: ['dashboard','user','about'],
  Manager: ['dashboard','manager','about'],
...
function setAuthMode(mode){
  ...
  document.getElementById("auth-sub").textContent = signup
    ? "Create a citizen account — a Manager can promote it later if needed."
...
document.getElementById('li-submit').addEventListener('click', async ()=>{
  ...
  try{
    if(authMode === "signup"){
      ...
      const { profile } = await apiPostJSON('signup.php', { name, email, password: pass });
...
document.querySelectorAll('nav.roles button').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    if(btn.dataset.role==='reportslog') renderReportsLog();
...
async function startCamera(){
  ...
  try{
    ...
    setCamStatus("Camera permission denied — use Upload instead");
  }
}
...
document.getElementById('u-locate').addEventListener('click', ()=>{
  ...
  out.textContent = "Pinpointing your exact location…";
  navigator.geolocation.getCurrentPosition(
    pos=>{
      const { latitude, longitude, accuracy } = pos.coords;
      setPickedLocation(latitude, longitude, `from device GPS · ±${Math.round(accuracy)}m accuracy`);
      if(locationMap && locationMarker){
...
function renderReportsLog(){
  ...
  tbody.innerHTML = list.map(r=>`
    <tr>
      <td class="mono">${r.createdAt || r.ts}</td>
...
function exportReportsCSV(){
  ...
  const rows = reports.map(r => [
    r.createdAt || r.ts,
    `L${r.level} - ${LEVEL_INFO[r.level].label}`,
    r.context,
    r.status
  ]);
  ...
  a.href = url;
  a.download = `kubli-reports-${new Date().toISOString().slice(0,10)}.csv`;
...
document.getElementById('log-export')?.addEventListener('click', exportReportsCSV);
